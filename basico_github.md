...
Estos están creados con "Git" como base.
 ...
 ![Carpeta .git](/images/carpeta_git.png)
...
Con este comando añadimos todos los cambios que hemos realizado desde el último commit al "Staged area".

![Cambio](/images/cambio.gif)

En este fragmento muestro (desde la interfaz gráfica de Visual Studio Code) como al realizar un cambio en un archivo del proyecto lo he añadido a el Staged Area manualmente.

### ¿Qué es el "Staged Area"?

El Staged Area es la "fase intermedia" que hay para cada vez que hacemos un **commit**, es decir, guardar los cambios realizados.
...
Con este comando "confirmamos" los cambios añadidos en el staged area. Al hacer un commit se crea una nueva versión de nuestro proyecto.
...
Con este comando vemos los cambios actuales, archivos nuevos y modificados que se encuentran en el staged area.

> `git reset`

Si usamos este comando sin especificar un archivo descartará **todos** los cambios del staging area pero el código de nuestros archivos se **mantiene intacto**. Para hacer esto también podemos usar `git restore --staged` 

Hay algunos parámetros del git reset que podemos usar:
* > `git reset [archivo]`

Saca del staged area el archivo que queramos manteniendo el resto de cambios.
